...
class ControllerModulePavblog extends Controller {
	...
	protected function validateBlog() {
		...
		if (utf8_strlen($this->request->post['pavblog_blog']['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->post['pavblog_blog']['blog_id']) && $url_alias_info['query'] != 'blog_id=' . $this->request->post['pavblog_blog']['blog_id']) {
					$this->error['warning'][] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->post['pavblog_blog']['blog_id'])) {
				$this->error['warning'][]= sprintf($this->language->get('error_keyword'));
			}
		}
	...
	public function blogs(){
		...
		$this->mdata['view_link'] = HTTP_CATALOG;
		...
		$this->mdata['sort_title'] = $this->url->link('module/pavblog/blogs', 'token=' . $this->session->data['token'] . '&sort=bd.title' . $url, 'SSL');
		$this->mdata['sort_created'] = $this->url->link('module/pavblog/blogs', 'token=' . $this->session->data['token'] . '&sort=b.created' . $url, 'SSL');
		$this->mdata['sort_date_modified'] = $this->url->link('module/pavblog/blogs', 'token=' . $this->session->data['token'] . '&sort=b.date_modified' . $url, 'SSL');
		$this->mdata['sort_status'] = $this->url->link('module/pavblog/blogs', 'token=' . $this->session->data['token'] . '&sort=b.status' . $url, 'SSL');
		$this->mdata['sort_position'] = $this->url->link('module/pavblog/blogs', 'token=' . $this->session->data['token'] . '&sort=b.position' . $url, 'SSL');
		$this->mdata['sort_hits'] = $this->url->link('module/pavblog/blogs', 'token=' . $this->session->data['token'] . '&sort=b.hits' . $url, 'SSL');
